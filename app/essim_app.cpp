...
void print_help() {
    ...
    printf(" -w : Width of yuv. Max Width <= 7680, to avoid huge precision loss in eSSIM score \n");
    printf(" -h : Height of yuv. Max Height <= 4320, to avoid huge precision loss in eSSIM score \n");
    ...
    printf(" -mode : Can be 0 -> SSIM_MODE_REF, 1 -> SSIM_MODE_PERF_INT, 2 -> SSIM_MODE_PERF_FLOAT. Default is 1 \n");
    printf("\n Example cmd : \t");
    printf(" -r /mnt/d/InpYuvPath/xyz.yuv -d /mnt/d/ReconYuvPath/abc.yuv -w 1280 -h 720 -bd 10 -wsize 16 -wstride 8 -mink 3\n");
    printf("\n");
}
...
int main(int argc, char **argv) {
        ...
        while((i < (uint32_t)argc) && (strcmp(argv[i], "") !=0 )) {
            if(strcmp(argv[i],"-r") ==0) {
                ...
                if(Mode != 0 && Mode != 1 && Mode != 2) {
                    Mode = 1;
                    std::cerr << "Invalid mode. Using default eSSIMMode i.e 1 (SSIM_MODE_PERF_INT)"
                              << std::endl;
                }
            }
                ...
                std::cerr << "ESSIM Minkowski Pooling: " << essim_mink_value << std::endl;
            }
    ...
    std::unique_ptr<std::ostream, void(*)(std::ostream*)> outStream(nullptr, [](std::ostream*){ /* do nothing */ });
    ...
    for(FrNum = 0; FrNum < TotalNumOfFrames; FrNum++) {
        ...
        if(BitDepth == 8) {
            ...
            ssim_compute_8u(&SSIMScore, &ESSIMScore, InpYuvBuff, stride,
    ...
    std::cerr << std::endl;
    std::cerr << "Average eSSIM: " << std::fixed << std::setprecision(4) << finalESSIM << std::endl;
    std::cerr << "Average SSIM: " << std::fixed << std::setprecision(4) << finalSSIM << std::endl;
